...
public class RedisReentrantLock {
     ...
     * @return 是否加锁成功 1 为成功
     */
    public Integer createLock(final String lockKey ,final String lockValue, final Long millisToLock ,final Long startTime){
        ...
        Object execute = stringRedisTemplate.execute(redisScript, keys, values);
        ...
        Thread guard = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                for (;;){
                    ...
                    Object res = stringRedisTemplate.execute(redisScriptDelay, keys, values);
                    System.out.println(Thread.currentThread().getName()+"执行守护线程lua脚本");
                    ...
                    if(System.currentTimeMillis() - startTime >= awaitTimeout){
                        System.out.println(Thread.currentThread().getName()+"持有锁超时,结束守护线程");
                        return;
                    }
    ...
    public void unLock(){
        ...
        try {
            if (Objects.equals(null,lockData)){
                throw  new IllegalMonitorStateException(Thread.currentThread().getName()+"当前用户未持有此锁:"+lockId);
            }
            //减少重入次数
            ...
            if (i < 0){
                throw  new IllegalMonitorStateException(Thread.currentThread().getName()+"当前用户重入此锁有误:"+lockId);
            }
            //执行解锁的lua脚本，根据返回值判断是否成功
            ...
            Object result = this.stringRedisTemplate.execute(redisScript,keys, values);
            int res = Integer.parseInt(String.valueOf(result));
            if (Objects.equals(null,result)|| res == 0){
               throw new IllegalMonitorStateException(Thread.currentThread().getName()+"当前用户删除此锁有误:"+thread.getName());
